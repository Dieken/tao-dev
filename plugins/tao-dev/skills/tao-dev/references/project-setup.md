...
2. 优先采用既有 CI 和项目命令，检查 watch 模式、外部服务与副作用。非空项目提供“接入现有工具”“补充选中工具”“仅查看建议”；空项目待技术栈明确后再接入，但延后不是静默的：记录延后了哪些检查、重新接入的触发点（技术栈确定，通常即 design 完成），以及谁依赖它。已有授权覆盖时直接推进。
...
6. 核对生成目录与 `/.worktrees/` 的忽略规则，保留 `.tao/config.toml`、工作流状态和正式 handoff；运行 `tao doctor` 与获准基线检查，报告写入及结果，已有事项接 `continue` 动作，否则接 `new` 动作。重复 `setup` 动作只检查、补充；因此新工具随对应任务增量补配，不在首次接入时预配还不存在的工具，避免一批 `not_run` 长期卡住交付判定。
...
include、exclude 从项目根按 Python Path.glob 展开，以包含集合减去排除集合；`*` 不跨目录，`**` 含零层或多层。排除整棵子树用 `vendor/**/*.md`，不按路径尾部匹配。只接受项目相对的斜杠模式：带盘符、以分隔符开头或含 `..` 的模式按配置错误拒绝，判定与运行平台无关。普通 README、运行 prompt、模板与 manifest 不因文件后缀自动纳入。
