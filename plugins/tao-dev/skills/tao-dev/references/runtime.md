...
显式 install 包含完整插件、核心与出版依赖，以及所选客户端范围内配置的安装授权。独立 setup 在已有依赖准备授权内执行，目标明确时无需反复询问类型。普通命令与 hook 不下载依赖、不隐式运行 setup。网络依赖准备使用 PyPI；企业离线分发可提供与清单哈希匹配的 wheel。安装过程屏蔽可能将写入重定向的 pip 配置，环境缺少合适包时说明原因，不改为源码构建或未锁定安装。
...
已管理安装重复运行相同客户端和作用域的 install，会刷新来源并升级，复用安装身份与完整且匹配的环境；不指定新来源时沿用记录中的来源。依赖、Python 或平台改变后重新运行 doctor，并在已有授权内执行 install 或 setup。
