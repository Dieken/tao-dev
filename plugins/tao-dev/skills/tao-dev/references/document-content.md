...
questions 只写尚未确定的目标、范围、方案或验收条件；没有则写“无”。本次新增或修改的需求确需延后时，在此用 `- deferred: <REQ_ID> — <触发条件与后续承接>` 逐行声明；缺声明时完整验证报未承接，缺触发条件时报声明无效——它是豁免声明而非扫描范围，忘记写会失败而不是静默缩小核对面。已知范围归 scope，验证结果及缺口归 verification，后续工作归 tasks；文档没有对应章节时引用相关计划或设计，不另加章节。
