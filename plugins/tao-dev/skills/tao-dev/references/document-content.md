...
字段顺序、必需项、编码和允许的引用类型见注册表的 `tasks` 节。字段行缩进两个空格；relates 至少一项，relates 与 depends_on 均不含重复成员。verify 为非空单行文本，写实际检查办法与预期结果，任务说明修改范围与可验证结果，不能仅写“完成某模块”；depends_on 不得成环。影响需求或方案的未知先安排求证任务，下游实现依赖其结论，不把假设写成已定承诺。
