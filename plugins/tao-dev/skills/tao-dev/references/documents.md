...
格式注册表权威定义 profile 名称、元数据字段、章节顺序、条目字段和模板位置；本文解释内容要求、引用与完成语义。模板是这些规则的写作起点，不能反过来修改规则以适配模板。注册表是机器可读的格式目录，**不是完整 JSON Schema 或 Markdown 校验器**。源文件校验器读取同包注册表，用 Markdown AST 校验结构、关系与源位置，入口与保障边界见 [诊断规程](document-diagnostics.md)。工作流 CLI 的集成另行验收。
...
操作使用项目写入锁与单文件原子替换，先保存退役记录再移除正文。两步不构成跨文件原子事务：若中断在两步之间，重复定义诊断会阻止把中间状态当作有效索引，ID 不会因源文件删除而失去记录。检查现场后可用相同参数重试完成；不要自动删除锁、改写不一致的记录或把检测到的重复定义隐藏掉。常规文件正文调整仍保留在 VCS 差异中，不另建事务台账。
...
4. CLI 可用后在修改文档时执行 `tao verify --only docs`，交付前按流程执行完整验证。当前可运行源文件校验器，另按实际项目工具检查行为，并注明各自范围。
