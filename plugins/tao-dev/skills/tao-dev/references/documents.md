...
一次检查或交接没有关联变更时，分别放在 `docs/evidence/` 或 `docs/handoffs/`。长期文档使用语义文件名；变更文件的 date 为 `created` 去掉连字符，seq 为三位日期序号，slug 为小写 ASCII 单词与连字符。附件沿用计划文件去掉 `.md` 的目录名。

日期序号使用项目局部 `.tao/name-sequences.json` 保存分配上限并纳入 VCS，取 `max(登记上限, 当前文件最大序号) + 1`，从 001 到 999，不回填空缺。分配时加锁、原子写入并排他创建；耗尽报错，跨分支冲突在集成时调整文件名，ID 不变。工具未实现时由单个写入者维护；该登记不参与随机 ID 生成。

规格定义承诺，计划引用这些定义并说明本次差异。默认计划依次包含目标与边界、规格引用、设计、任务、验证、未决问题，不要求每个变更分别创建 requirements、design、tasks 三份文档。局部文案／格式修正且不改变行为、接口、权限、持久数据或运行约束时，可以不新建计划。

超过 20 个正式任务时，将全部任务移到任务附件；设计超过 120 个非空源文件行时移到设计附件。计划分别通过 `tasks_doc`、`design_doc` 引用附件的 DOC ID；附件通过 `change` 引用计划定义的 CHG ID。原章节只留对应 `{need}` 引用和职责说明，不能保留第二份任务或设计正文。拆分不改变条目 ID；计划及任务附件合起来至少有一个 TASK。

同时编辑者确实会写同一区域时按拥有的子系统拆附件；单个 design_doc 或 tasks_doc 入口可引用其他命名附件，但正式定义仍只有一处。约束至少两个变更或改变系统级接口／数据／权限契约的决定写成长期 ADR。计划仍超过 300 个非空源文件行时抽取完整参考材料，不拆单个条目或压成长行来凑阈值。阈值可由项目统一配置，不能逐次临时更改。
...
文件导航用项目内相对 Markdown 链接。发布时条目标题锚点使用完整 ID，二级章节使用 `<DOC-ID>--<section-key>`；正式分享入口由书籍索引生成，不依赖自动标题 slug。出版尚未实现时不要宣称移动后的 URL 已可用。
...
evidence 可选，但勾选完成时必需，为指向可复查报告的单个 Markdown 链接；有证据不自动代表完成，checkbox 也不代表已合并或发布。原始报告可先直接链接；使用 tao.evidence 摘要时，其中列出报告、输入、结果与限制。依赖图、执行批次、反向引用和覆盖表由字段生成，不另手工维护 JSON waves 或重复状态。
...
原始输出放 `artifacts/<run-id>/`，默认不纳入 VCS；书籍输出放 `artifacts/book/`。需要持久引用时挑选摘要与必要小报告纳入变更 evidence 目录，大文件放指定持久产物库并记录校验和与期限。证据输入指纹排除生成报告、证据自身和可重建缓存，但必须包含影响行为的代码、规格、配置和测试。报告不可读取、已过期或输入变化时，不能继续把旧证据当作当前通过依据。
