...
受管理文档记结论与判断依据，不记执行者的操作经过。判据是未来读者判断产品或某个决定时是否需要该内容：支撑阈值、验收或取舍的测量值与事实属依据，须保留；尝试次数、返工顺序与已修复且不改变产物的失误属过程，归提交消息或检查点。执行记录只写受检对象、观察办法与实际结果。
...
同一版本的正式索引只允许一个定义；示例、档案和测试样例不注册当前定义，多版本出版分别建索引。退役记录纳入 VCS 并参与查重，无法恢复历史基线时不能宣称查出了所有历史删除。外部项目的正式引用需显式导入索引，无可用索引时报告 unresolved。
...
正文引用完整 ID 用 `{need}` 角色，引用文件用相对 Markdown 链接，行号写在链接外；审查快照路径只说明受检输入，不代替正文导航。校验器只对这两类遗漏给出 warning，不自动注册引用，也不为消除 warning 编造链接；缩写 ID 与引用意图仍须人工核对。

## 检查与修正

填写当前模板、替换全部变量并分配新 ID 后，运行 `tao verify --only docs`。普通局部文档维护检查受影响内容；已接入完整交付流程的事项按 [开发动作](workflow-actions.md) 执行交付检查。
