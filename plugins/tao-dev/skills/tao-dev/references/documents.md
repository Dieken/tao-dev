...
创建、修改、审查 tao 管理的规格、设计、计划、任务或证据时读取本文，再读取本次实际使用的 profile 所对应的模板；同一任务确实修改多类文档时才读取多个模板。普通代码、测试或配置修改没有文档同步职责时不读取本文。文档格式由同包的 [格式注册表](../assets/document-profiles.json) 和模板提供。
