...
交付时挑选必要的摘要和小型报告进入对应变更的 `evidence/` 并纳入 VCS；大型报告、视频、构建包可进入项目指定的持久产物库。证据摘要记录关联 ID、受检输入引用、命令与环境、结果、内容校验和、稳定位置及保存期限；不要保存凭据。未配置持久产物库时可以选择归档必要文件，不能只写一个即将过期的 CI 链接。

### 受检输入：优先引用版本，不复制文件清单

默认在 evidence 的 inputs 章节、现有 fingerprint 字段内记录输入引用，不新建只有版本号的文件，也不要求 inputs.sha256 或逐文件哈希清单。该字段表示可核实的输入标识，不限定为哈希；FINGERPRINT 模板变量按下面的规则填写。
...
运行前后核对版本、受检范围及影响执行的版本外输入；确认输入变化则标记 stale，无法确认一致性则不可复用，不把相同版本号当作充分条件。证据保存提交可以晚于受检提交；仅新增报告不要求重跑所有检查。排除生成报告、证据自身与缓存，但不能排除实际影响行为的源文件。清理不能删除仍被有效证据引用的唯一副本；无法读取或已过期的证据报告为不可复核，不继续显示通过。
