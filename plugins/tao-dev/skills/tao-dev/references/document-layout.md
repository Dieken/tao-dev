...
项目内容与工具配置按用途分开：规格、计划、证据和退役记录属于项目文档资产，默认在 docs/；.tao/ 只放控制 tao CLI 或 skill 行为的配置，配置无实际需要时不创建该目录。移除工具不应带走文档资产。可重建索引、缓存和短期锁默认放 artifacts/cache/tao/，沿用生成产物的忽略规则，不能把这里作为正式记录的唯一保存位置。
