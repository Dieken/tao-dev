...
已登记为 running 的轮次必须先核对进程和实际结果，不再次派发。失败尝试也占用轮次。开始时间和已用轮次保存在工作流状态中，换会话不清零。确有新证据、范围扩展或用户新预算时，可显式传 --max-rounds 与 --budget-seconds 调整累计上限，记录决定；不能由 agent 为了继续尝试自行增加。
