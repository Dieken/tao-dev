...
本轮将新建的正式报告、裁决或专用导航，在预览时用可重复的 `--output <项目相对 Markdown 文件>` 逐个预留。路径必须尚不存在且不属于跟踪输入，不接受目录或 glob；预览的 output_files 与排除项一起确认。开始前仍须不存在，开始后仅这些文件不参与输入摘要；历史报告、其他新文件、源码和已有导航变更仍使输入过期。发布挂接若需修改已有导航，将该修改纳入受检版本或作为后续文档变更，不把它伪装成本轮新输出。批次自身的记录不改变该批次的受检输入，但父级导航入口不属于批次记录：它若在 review-begin 之后才加入，该轮即过期，进入 finish 的条件随之不成立。因此建批次时先建好批次导航页并把它挂进父级目录，再预留报告与裁决路径并开始本轮。无输出声明的旧记录沿用原绑定。
