...
项目源码已公开托管于 [GitHub](https://github.com/Dieken/tao-dev)，目标客户端是 Codex CLI 和 Claude Code CLI，已测组合与限制见下方支持状态。

### 1. 在目标项目中使用规程

克隆仓库后，在目标项目的客户端会话中提供本地 skill 文件的位置，并描述目标，例如：

> 请先读取 [SKILL.md](plugins/tao-dev/skills/tao-dev/SKILL.md)，用 tao-dev 制定计划：让任务列表支持按“待办、进行中、已完成”状态筛选。
...
| 你可以这样说 | 操作意图 | 得到什么 |
|---|---|---|
| 用 tao-dev 为任务列表的按状态筛选功能制定计划 | new | 已填写目标、范围、已知验收和下一步的草稿 |
