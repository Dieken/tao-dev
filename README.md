...
分发内容中的 `SKILL.md`、运行参考、模板和脚本单独组织；根据已确定的需求提炼必要内容，明确对应关系，并通过测试检查一致性。发布采用明确的文件清单，不将整个开发仓库打包；运行材料不得依赖 `docs/` 或本地研究目录。产品模板维护在 skill 的 assets 中；内部文档的章节与研发条目不属于这些模板。
