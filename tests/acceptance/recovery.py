...
def prepare(directory, plugin):
    entry = plugin / 'skills/tao-dev/scripts/tao.py'
    def tao(*arguments):
        p = subprocess.run([sys.executable, str(entry), '--project', str(directory), '--format', 'json', *arguments], capture_output=True, text=True)
        if p.returncode:
...
''')
    ...
    failure = subprocess.run([sys.executable, str(entry), '--project', str(directory), 'verify', '--only', 'code', '--format', 'json'], capture_output=True, text=True)
    if failure.returncode != 1:
        raise RuntimeError('Expected a real failing baseline: ' + failure.stdout)
    labels = json.loads((plugin / 'skills/tao-dev/assets/locales/en.json').read_text())
    ...
    draft.write_text(re.sub(r'\{\{([^}]+)\}\}', lambda m: values[m[1]], (plugin / 'skills/tao-dev/assets/templates/handoff.md').read_text()))
    draft.write_text(draft.read_text().replace('status: draft\n', 'status: draft\nchange: ' + created['ids']['CHG_ID'] + '\n'))
