"""Mutation checks that prove behavior oracles detect known violations."""
...
def _event(number, kind, **data):
    return Event(f"event-{number}", 1, kind, "mutation", float(number), data)
...
def run_mutation_suite(catalog):
    """Inject eight workflow violations and require their target rules to fire."""
    ...
    cases = (
        ...
        ("mutation-after-verification", "completion-claims-have-evidence",
    ...
    results = []
    for mutation_id, scenario_id, rule_ids, baseline, mutate in cases:
        scenario = scenarios[scenario_id]
        before = _status(evaluate_deterministic(catalog, scenario, baseline), rule_ids)
        after = _status(
            evaluate_deterministic(catalog, scenario, mutate(baseline)), rule_ids)
        results.append(MutationResult(
            mutation_id, scenario_id, rule_ids, before, after,
            before == "pass" and after in {"fail", "blocked"},
        ))
    missed = tuple(item.mutation_id for item in results if not item.detected)
