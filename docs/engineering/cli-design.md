...
结构化度量适配用独立样例验证。Claude 优先累加 modelUsage 的主模型与辅助模型计数；多个 result 可能为累计值，当前记为未知。Codex input_tokens 已含缓存命中，不重复相加。Coverage.py、Ruff 和模型用量解析须区分原始值、累计值与未知状态；不能把估算写成供应商账单。receipt 只保存合并输入、环境和策略摘要及执行结果，不持久化逐文件哈希表；记录写入使用项目锁和原子替换。
