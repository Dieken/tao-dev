...
`review-end` 在登记前校验所提交的报告是否为合格的受管理文档，不合格则拒绝登记。此前它只核对文件存在与输入绑定，把 schema 与引用检查留给调用方，结果是报告可以先被登记、事后才发现要改格式，改完其摘要便与登记值不符而该轮已无法重新登记。登记的是结论与材料的对应关系，材料不合格时这个对应关系本就不成立，因此把校验前移，而不是继续依赖调用方记得先跑。
