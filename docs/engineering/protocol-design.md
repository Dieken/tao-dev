...
用户的目标、硬性约束与实现建议分别处理：前两者约束方案，后者需分析收益、必要性及维护代价。允许提出不采用、缩小或延期的建议；用户明确权衡后尊重其决定，不重复争论，也不以已有投入为继续扩张的理由。执行文本见 [审查规程](../../plugins/tao-dev/skills/tao-dev/references/review.md)。
