...
安装顺序为参数与边界检查、取得来源、准备两套锁定依赖、原生安装与范围配置、记录运行绑定、原生加载与 doctor 检查、打印汇总。共享 tao CLI 保存在客户端配置目录的 tao-dev/cli/，启动器默认写入 ~/.local/bin/；可用 --bin-dir 指定目录，安装报告提供绝对路径，不修改用户的 shell 配置。准备失败不宣称成功；重试复用完整运行环境。升级复用安装身份和运行根；原生缓存由客户端管理。更新前保留原有源码、所有已知旧插件缓存与所选原生配置项，doctor 通过后才原子更新共享命令；失败时恢复配置和缓存，恢复不完整则将记录标记为 preparing 并说明。卸载先修改所选范围的激活配置，再判断是否仍被其他安装引用，最后删除确定归属的 tao-dev 目录；共享配置仅移除本次管理的键。
