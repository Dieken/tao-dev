...
需要 uv 和 Python 3.11–3.14。开发依赖由 pyproject.toml 声明、uv.lock 锁定，环境位于忽略的 .venv/。出版链接测试通过 Python 包 mini-racer 执行生成的浏览器脚本，随开发依赖安装，不需要 Node.js，也不加入插件运行依赖。
...
修改版本用 `just bump-version`，省略参数取下一个次版本，追加 `dryrun` 只报告。它不按记忆列出位置，而是在已跟踪的树中搜索当前版本，把命中分为下表编辑、生成器重写、`docs/plans/` 历史记录三类，遇到无法归类的命中就停下并要求逐项决定。下表记录它编辑的位置及理由：
...
发布前运行依赖导出的 `--check`、相关回归和打包检查，并核对上述版本一致；其中 `--check` 只校验生成的运行策略与依赖清单，不检查全部 manifest 和 CLI 版本。生成的公共 manifest 由打包脚本继承 Codex manifest 的版本，无需单独维护。回归与验收脚本中的版本字面量会被上述守卫报为无法归类：逐项决定它跟随发布，还是改用不会与发布版本碰撞的合成值。
