...
面向使用方项目的写法见 [正文写法](../../../plugins/tao-dev/skills/tao-dev/references/document-content.md)。
...
| 临时原始输出、测试样例、外部历史资料 | 明确范围外；不得计入正式文档覆盖或直接冒充交付结论 |

源校验器在 AST 正文中提示未链接的完整 ID 和独立代码片段中的 Markdown 路径，使用 TAO-REF-005／TAO-LINK-002 warning。已解析的 TASK 定义和关系字段、链接显示文字、围栏代码及计划 questions 中的 `- deferred:` 声明行不重复提示；声明行按原样逐行比对，其中的需求 ID 是记录本身而非正文引用，改写成引用语法反而使声明无效。不从裸文字自动建立关系。带行号或行区间的文件引用指向文件中的位置，Markdown 链接无法表达它（链接里的行号由 TAO-REF-004 按错误报告），因此两种形式一律按代码引用处理；evidence 文档是过去状态的记录，登记时其摘要已钉入工作流状态，正文不再收到这两条提示。该检查是引用意图的提示，不能可靠识别缩写、普通文本中的任意路径或所有语义引用；内容审查及实际 HTML 链接核验仍必需。
...
一条关系只维护其发出方：反向引用、需求覆盖表和汇总由索引生成，生成文件不得成为另一份人工维护的真相。检查目标存在、关系类型、任务依赖无环和替代关系无环。标为 superseded 的对象必须能找到至少一个替代者；引用 retired／superseded 对象给出复查提示，合法的历史解释不必全部阻断；evidence 文档引用的是它写下时存在的 ID，该提示对它不成立，因此不报。

出版层复用 taolib 的源文档索引，以同一份定义和关系生成条目引用、稳定锚点及 refs/ 解析页。PDF 和第三方编辑器表现不由 HTML 构建结果推导。

中文搜索使用相邻字符对建立索引，不依赖分词组件；单汉字查询通常无结果。索引随正文增长并由各页加载，构建器验证生成脚本能否加载；算法和加载机制属于出版器维护职责，使用方项目的操作指导仅保留查询限制和检查要求。
...
spec_docs 同时是计划的需求范围声明：完整验证据此要求这些规格里的正式需求都有任务承接，并拒绝关联范围外的需求，范围不另设一份人工清单。该比对只在完整验证进行，草稿校验不受影响；它不需要基线，与按基线差异枚举的那一角度分别成立——需求进入基线后差异为空而范围仍在。未声明 spec_docs 的计划报范围未声明，不从文档树推断；批准闸口已要求该字段存在。
...
| TAO-DOC-003 | error | 成对的契约标记未闭合、缺少一端或出现在不允许的章节；定位开启处与期望的闭合位置 |
...
成对标记单独成规则号，因为它的失效方式与缺章节不同：未闭合的开启标记既不缺章节也不重复键，结构看似完好，被改变的是随后内容归属哪个契约区段。这类错误不在此处报出时，会转由下游的批准或完成条件以无关原因失败。上述规则号稳定，措辞可以改善。输入语法损坏时先报告根错误，抑制由它引起的大量误报。退出码：`0` 无 error（可含 warning），`1` 有文档 error，`2` 工具或配置故障、检查未完成。发布就绪检查另评阻断问题，不能与格式退出码混为一谈。


错误传播先保留根诊断，抑制派生误报；使用方项目的修正方法见 [文档规程](../../../plugins/tao-dev/skills/tao-dev/references/documents.md)。
...
新增 profile 或结构字段时，同时修改注册表、运行规程、模板和独立测试样例。运行调用方读取同包契约，不从开发文档动态推断格式。
