...
源校验器在 AST 正文中提示未链接的完整 ID 和独立代码片段中的 Markdown 路径，使用 TAO-REF-005／TAO-LINK-002 warning。已解析的 TASK 定义和关系字段、链接显示文字、围栏代码及计划 questions 中的 `- deferred:` 声明行不重复提示；声明行按原样逐行比对，其中的需求 ID 是记录本身而非正文引用，改写成引用语法反而使声明无效。不从裸文字自动建立关系。该检查是引用意图的提示，不能可靠识别缩写、普通文本中的任意路径或所有语义引用；内容审查及实际 HTML 链接核验仍必需。
