...
正式 ID 的格式为 `类型_YYYYMMDD_16位随机串`，例如 `REQ_20260914_7M2K9X4C6P8R3T5V`。这是 tao-dev 的项目格式，采用 Crockford Base32 字符集与 80 位随机部分；不是 UUID 或标准 ULID。

```regex
^(DOC|REQ|UC|ADR|TASK|CHG|EVD)_[0-9]{8}_[0-9A-HJKMNP-TV-Z]{16}$
```

| 片段 | 含义 |
|---|---|
| `DOC` 等类型前缀 | 标识条目用途，完整类型见术语表 |
| `YYYYMMDD` | 首次分配 ID 时执行环境的本地日历日期，固定八位；不表示修改时间或对象的历史起源 |
| 16 位随机串 | 字符集为 `0123456789ABCDEFGHJKMNPQRSTVWXYZ`，每位 5 比特，共 80 位；统一大写，不使用 I、L、O、U |

字符集与随机长度借鉴 [ULID 规范](https://github.com/ulid/spec#specification)，日期直接使用可读数字，未采用 ULID 的毫秒时间编码或单调递增机制。含三字母类型前缀的完整 ID 为 29 个字符，TASK 类型为 30 个字符。日期只提供时间线索；跨环境时钟差异不会改变已分配的 ID，也不保证 ID 排序就是事件顺序。

正则只检查外形。校验器还须按真实日历解析日期，拒绝不存在的日期，并检查完整 ID 的唯一性、条目类型和引用关系。不要套用 ULID 首字符只能为 0—7 的限制：这里的随机串是完整 80 位，首字符可以是上述字符集中的任意字符。

每次生成独立的 10 字节安全随机数据，例如使用 Python `secrets.token_bytes(10)`，按大端顺序编码为恰好 16 个 Base32 字符，保留前导零。不能用普通伪随机生成器、时间戳、内容哈希、截短的 ID 或流水号代替这 80 位随机数据。生成器检查当前索引和退役记录；遇到完整 ID 碰撞时重新生成，最多尝试 3 次，仍冲突则报告错误。该生成过程无需日期序号登记或跨仓库共享编号服务。

在独立均匀随机假设下，同一日期、同一类型共生成 n 个 ID 的碰撞概率约为 `n(n−1) / (2 × 2^80)`；100 万个时约为 `4.1 × 10^-13`。这是小概率条件下的生日界估算，不取代构建查重，尤其不能掩盖复制导致的重复。

ID 只分配一次，禁止按标题、路径、内容或章节号重新计算。日期固定为 ID 分配日，不随元数据日期纠正、翻译或环境变化重算；无需强制它与导入对象的 `created` 相同。类型前缀是标识符的一部分；更改对象类型需要新对象与迁移关系。日期＋序号仅用于可读文件名，分配规则见文档组织专篇；正式引用不另设短 ID。
...
```markdown
- [ ] `TASK_<YYYYMMDD>_<16位随机串>` 可验证的工作结果
