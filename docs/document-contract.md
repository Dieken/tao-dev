...
日期使用 `YYYY-MM-DD`，取记录时执行环境的本地日历日期；无需配置项目时区或统一换算为 UTC。`created`、`updated`、`retired_on` 等天级字段不表示精确时刻，已有日期不因换机器或时区变化而重算。导入文档若无法确认创建日期，记录首次纳入本体系的日期并在导入说明中标明，不能假造原始日期。`created` 不因移动、翻译或普通编辑变化；`updated` 不因构建、格式化或工具读取而更新，可省略以减少维护噪声。运行 SKILL、agent 和客户端 manifest 不增加其上游格式未支持的日期键。

需要精确时刻的执行、证据或事件记录使用 ISO 8601 的 [RFC 3339 §5.6](https://www.rfc-editor.org/rfc/rfc3339.html#section-5.6) 格式：包含完整日期、`T`、时分秒以及数值 UTC 偏移量或 `Z`，可按实际精度保留小数秒，例如 `2026-09-14T10:30:00+08:00` 或 `2026-09-14T02:30:00Z`。生成器使用大写 `T`／`Z`，不输出缺少偏移量的精确时间戳。天级日期保持简洁，追溯修改时刻可查 VCS 历史；执行时刻仍以实际执行记录为准。
