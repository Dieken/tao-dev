---
schema: tao.document-contract/v0.1
...
本文是 tao-dev 的内部文档格式设计，供维护者实现解析器、schema、诊断和模板时使用；不属于分发给第三方项目的 skill 文档。格式在本项目开发文档中自举验证，正式机器 schema 和交付模板另行实现。目标是兼顾 Markdown 的可读性与结构、身份、关系的可检查性。
...
| 身份与关系 | 唯一性、引用目标、类型、依赖环、状态约束 | 可确定索引范围内的一致性 |
...
文件采用 UTF-8，第一行开始一个 YAML frontmatter，正文恰有一个一级标题。元数据拒绝重复键、自定义 YAML tag 和未知字段；读取采用安全模式。v0.1 允许字段如下：
...
章节键匹配 `[a-z][a-z0-9-]*`，在当前文档内唯一，调整标题文字不改变键。数字是阅读顺序，不是身份。键用于结构校验与内容提取，不作为跨文档全局 ID；需要长期引用的内容使用条目 ID。三级及以下标题自由组织，不跳过中间标题级别。
...
ID 在创建时用 UUID 库生成一次，禁止按标题、路径、内容哈希或章节号重新计算。多仓库无需共享编号服务；构建仍查重，不能用概率唯一为复制错误辩护。类型前缀是身份的一部分；更改对象类型需要新对象与迁移关系。

改名、移动、补充说明以及同一需求的版本演进保留 ID。拆分为独立需求、替代或废弃时保留历史记录及关系，不回收旧 ID。ID 只标识对象，引用某一历史内容需要同时指定版本。
...
正式需求、决策、用例使用位于文档顶层的三反引号 MyST 风格块。块头为 `{req} 标题`、`{adr} 标题` 或 `{uc} 标题`；标题必须非空。字段置于正文之前，字段结束后空一行，关闭围栏单独一行。正文是 Markdown。v0.1 不允许正式条目嵌套、置于列表或引用块内。
...
字段键仅在对应条目的正文顶层生效；代码示例、引用中的同名文字不生效。v0.1 要求键后紧邻一个 Markdown 段落，可在段落内软换行；多段扩展需修订 profile。未知键、重复键、错序和缺少段落是结构错误。普通说明可以出现在字段段落之前或之后，但不能代替必需字段。机器检查结构和非空文本，是否充分由内容审查判断。

当前 protocol profile 至少包含一个 REQ、一个 ADR、一个 UC 和一个正式任务；其他 profile 不自动继承该数量要求。DOC 由 frontmatter 定义。CHG 和 EVD 已预留身份类型，但其正式定义格式尚未提供；初版不得为它们编造未声明条目块。
...
关系字段和任务中的 ID 列表也是正式引用。普通文件导航使用标准 Markdown 链接；标题锚点适合方便阅读，不作为业务条目的永久身份。代码注释引用完整条目 ID，由工具解析到正式位置；不要求在每个函数重复标注任务。

本项目开发文档的文件链接使用项目内相对路径，解析后须位于 tao-dev 项目范围内；不使用本机绝对路径、`file://` 地址或指向项目外文件的相对路径。外部资料使用公开上游链接；没有可用公开地址时只保留必要的来源名称和结论。文档应能随项目独立迁移，无须读者具备作者的研究目录。
...
正式任务必须使用 GFM 风格 checkbox，首行包括完整 TASK ID 与非空标题；条目下按顺序填写字段。示例只是语法说明，不注册任务身份：
...
`relates` 必需且至少关联一个 REQ 或 CHG；`depends_on` 必需，可空且只引用其他 TASK；`verify` 必需，为单行非空文本。v0.1 不支持嵌套任务与任务中再包含 checkbox。任务顺序用于阅读，执行顺序由依赖关系约束。
...
- **机器协议：** schema、章节／字段键、条目类型、ID、关系名、状态值、规则号、占位变量和退出码保持稳定英文／ASCII 形式。正文、标题、说明、任务目标和诊断消息可本地化；日期、数值等机器字段使用固定格式，地区显示由呈现层处理。
