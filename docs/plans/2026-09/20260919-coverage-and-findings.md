...
耗时与预算需单独说明。首次以配置策略执行时被单项超时终止：旧配置 `timeout_seconds = 720`，该次运行 720 秒被 SIGKILL，记为 `not_run`，不算通过也不算失败。随后不带封顶直接计时得 958.83 秒，据此把超时提高到 1200 秒、总预算提高到 1260 秒后重跑，实测 597.6 秒通过。同一套测试在本次会话中的四次测量分别为 430、598、605 与 959 秒，最快与最慢相差 2.2 倍，主要来源是机器负载而非用例数量；本批新增的 7 个用例均为启动子进程的 CLI 往返，成本高于进程内用例，但解释不了这个量级的差异。该波动未做归因拆分。
