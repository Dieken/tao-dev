...
维护回归一次：**493 passed、5 skipped，605.57 秒**，总覆盖率 72%。上一批基线为 490 passed、5 skipped、466.29 秒、覆盖率 72%（引自该批次记录，本次未复跑其版本）。新增 3 项即本批为三处代码缺陷补的红绿用例；5 项跳过仍是未启用的原生客户端安装探针，不计入通过。耗时增加 139 秒未做归因拆分：新增用例均为启动子进程的 CLI 往返，单项成本高，但本次未单独计时，也未排除机器负载影响。配置的两项检查均 passed，ruff 诊断 0。
...
{need}`REQ_20260914_4CS6P421MGW68PME` 要求的隔离场景行为验收未执行，是本批唯一确认的验收缺口。补足需在隔离实验项目内驱动真实客户端并两端分别记录，规模接近本批本身，应另立事项而非并入本计划；在此之前，本批规则的实际效果只有观察，没有验收。

本批新增内容使六份按需页净增 5,564 字节，已实测记录；但这是静态体积，是否影响实际加载成本与模型 token 消耗未做行为试验，上一批已确认「读取相关小节」仍导致整篇加载，因此该增量会整页计入。后续若需控制，应先度量再拆分。
