...
书籍章节迁移通过 documents.section_redirects 配置旧 DOC／章节键到当前 DOC／章节键的直接映射；目标来自同一索引，拒绝未知章节与跳转链，普通校验和退役共用约束。发布页生成按片段跳转及可读后备链接，不改文档的既有 ID 或维护第二份正文。

诊断使用完整英文模板及中文消息资源，参数与渲染文字分开；显式诊断参数覆盖项目界面配置，未配置时文档按自身 locale 呈现。启动层只依赖标准库，共用翻译资源；外部原始错误不改写，未知消息如实回退。自动检查翻译覆盖与占位集合，测试语言切换不改变规则、位置、退出码及检查范围。
...
本次执行与计量实现的验收记录（输入以本记录所在提交的运行源码、测试和配置为准）：项目检查实际生成 JSON 度量，缓存保留合并指纹、检查集合、工具／环境与原始修订引用；缺工具、预算不足、输入变化及损坏凭据均有独立反例。质量命令、覆盖率采集范围和日志位置由 .tao/config.toml 与 [维护检查入口](../../../tests/acceptance/quality.py) 固定；该次基线的普通回归不调用模型。覆盖率不等同需求满足，也没有凭空设置一个通过百分比。
...
Codex 0.154.0 的个人配置模式会持久化全局 projects 信任条目，调用级 trust override 也不能阻止，因此该模式继续禁用。新验收使用完全独立的客户端状态，并由宿主检查配置变化；不能采信模型“全局配置未变”的自述，或把目录内 skill 加载成功等同严格隔离通过。
...
退役命令的受检实现对应提交 `214112d`：180 项回归通过，Ruff 无诊断，代码检查实际耗时 366.659 秒，Sphinx HTML 构建通过。覆盖预览、替代对象、文档拥有条目、相邻任务、损坏记录、追加保存和写入中断恢复；完整交付仍受客户端与独立审查条件阻断。

章节重定向的受检实现对应提交 `bdddda0`：187 项回归通过，Ruff 无诊断，代码检查实际耗时 403.056 秒，项目书籍构建通过。用实际 JavaScript 执行验证旧入口、编码片段及重命名；无脚本时保留可读后备链接。新增诊断入口测试后，测试预算为 480 秒、总预算为 510 秒，保留原有检查与断言。

诊断本地化的受检实现对应提交 `b59329d`：198 项回归通过，Ruff 无诊断，代码检查实际耗时 397.883 秒，项目书籍构建通过。新增 11 项反例覆盖语言优先级、机器字段不变、无翻译回退、独立入口、hook 缓存、并发语言及缺少 tomllib 的启动诊断；系统 Python 3.9 实测能报告版本不受支持。检查期间有代码变化的一轮结果标为 stale，不作为最终证据。覆盖率观察值为 59.89%，隔离子进程的观测限制不变；完整交付仍受客户端任务和必需独立审查条件阻断。该提交的跨 Python／Linux 运行结果见 [运行环境验收](20260914-portable-runtime.md#DOC_20260914_3541E3H7SN5V7MS5--verification)。

维护审查执行器采用独立客户端配置和子进程访问令牌，不加载插件或携带刷新令牌。必须明确选择该模式，缺少选择及非法预算在读取审查输入前拒绝；超时终止进程组、清理令牌环境并脱敏日志，个人凭据或配置变化均不能生成可导入记录。新增离线反例覆盖这些边界；真实供应商复核仍须针对当前固定输入进行，不能沿用旧报告的 binding。
