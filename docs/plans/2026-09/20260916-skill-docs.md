...
- [x] `TASK_20260916_CXYJA4DC7KG1495E` 精简工程规程、术语、本地化与入口。
  - relates: ["CHG_20260916_XBKXDAF7KNN3Q0X3"]
  - depends_on: ["TASK_20260916_YVCPXGFRXBASZ9FA"]
  - verify: 保留工程取舍及非显然约束；核对场景读取范围、语言规则和模板一致性。
  - evidence: [验证记录](#DOC_20260916_VDYK8KF5Q3665DQY--verification)

- [x] `TASK_20260916_YD2WC2Q6K5T5TA77` 完成引用、行为保留、打包与最终验证。
  - relates: ["CHG_20260916_XBKXDAF7KNN3Q0X3"]
...
任务 7：术语、本地化诊断、引用解析与仓库共 26 项测试通过；198 个本地链接无缺失，skill 格式检查通过。已删基础缩写教学并收窄术语读取，保留工程取舍。
任务 8：全量 pytest 359 项通过、5 项原生客户端探针默认跳过；Ruff 与依赖导出检查通过。36 份受管理文档和 HTML 构建通过；公共包 128 个、Codex 包 104 个本地链接均在包内解析。独立保留审查无发现，三种阅读演练及新事项边界定向复核完成。

最终核对：入口与 18 份 references 的 Unicode 字符数从 63,979 降为 37,799，减少 40.9%；不将该值当作 token 或模型性能指标。原有 104 个正式 ID／引用全部保留，新增 10 个仅用于本计划。运行脚本、模板与 schema 无改动；验收辅助脚本仅更新参考文件分类。

受检基线为本仓库 f7b00bc3bb10829fa2e3cf12860c613b0f9d2e3a，最终工作区另含新事项分派说明和本计划验证记录；分发文本已与生成包逐字节核对。`.venv/bin/python -m pytest -q` 在本地 Python 3.12.13 环境执行，359 passed、5 skipped，耗时 445.02 秒。跳过项要求显式 TAO_TEST_NATIVE_CLIENTS=1，本次未运行原生安装探针；阅读演练是独立上下文静态应用，不声明跨供应商审查或新的客户端兼容性。
