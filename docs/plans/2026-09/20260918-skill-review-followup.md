...
验证过程中另外暴露两处与本批修正无关、但由同一次执行发现的缺陷，一并收在本计划：发布版本守卫被测试夹具的假阳性阻断，以及运行环境探测把负载当作环境损坏。随后声明 0.6.0。测试执行时间的优化已实测并放弃，理由见验证记录。
...
发布守卫与运行环境探测按同一原则处理：把瞬时或无关条件与真实故障区分开。版本守卫搜索已跟踪树并拒绝无法归类的命中，这项严格性要保留，因此假阳性修在夹具而非守卫——安装回执描述的是此前安装的副本，其版本本就不必等于开发中的版本，同文件的另一份回执已在用合成值。运行环境探测的时间上限只用于阻止无限等待，不是挂起检测器；超时表示未能完成检查，不构成环境损坏的证据，也不应以未翻译的内部命令行呈现给用户。
...
- [x] `TASK_20260918_7CKBPN9JW8K54TP1` 收敛审查模板的结果、分类与占位符约定。
  ...
  - evidence: [验证记录](#DOC_20260918_F8YYWKH5QY1KXPHE--verification)
- [x] `TASK_20260919_2HT0JVD784ZQF7TE` 解除发布守卫的假阳性，并把探测超时按负载而非损坏报告。
  - relates: ["CHG_20260918_94AXFPY6YV3H18VM"]
  - depends_on: ["TASK_20260918_7CKBPN9JW8K54TP1"]
  - verify: 空跑发布守卫确认无法归类的命中清空；核对新诊断的双语渲染与目录完整性；运行安装、运行环境与诊断本地化回归。
  - evidence: [验证记录](#DOC_20260918_F8YYWKH5QY1KXPHE--verification)
- [x] `TASK_20260919_9YQEVRS9GGC73DZ1` 声明 0.6.0 并核对各处版本一致。
  - relates: ["CHG_20260918_94AXFPY6YV3H18VM"]
  - depends_on: ["TASK_20260919_2HT0JVD784ZQF7TE"]
  - verify: 核对四处声明位置与两处生成文件一致；运行依赖导出核对；确认 CLI 报告的版本与锁文件来源。
  - evidence: [验证记录](#DOC_20260918_F8YYWKH5QY1KXPHE--verification)
...
发布守卫的空跑在夹具修正后只报告四处待改与两处待重生成，无法归类的命中清空；安装与运行环境回归 46 项通过。守卫本身未改动，因此它对未来出现的版本字面量仍然一律拒绝。

运行环境探测的时间上限由 10 秒改为具名常量 60 秒，并在超时时抛出受控消息。同一次并行试验还暴露该超时会从准备路径裸抛出去，经命令行兜底以 `str(exc)` 呈现：用户在一份 zh-Hans 报告里看到未翻译的英文句子和完整的内部命令行。修正后两个调用方都给出可重试的翻译诊断，新消息在 zh-Hans 与 en 下渲染正确且占位符一致。60 秒是界限而非实测值，最慢受支持平台上的冷导入上界未测量。

测试执行时间的优化已实测并放弃。串行基线 10 分 04 秒、总覆盖率 72%；启用 pytest-xdist 后墙钟降至 1 分 37 秒，但 490 项全部 ERROR，且覆盖率塌至 12%。失败根因是每个 worker 各自执行一次运行环境准备，并发下打爆上述探测超时；覆盖率塌陷则是 coverage 未追踪 execnet worker，而该数据被配置为验证策略的度量来源。调小并发可以调到通过，但那是在调时序，在核数更少的 CI 上不成立，因此不采用。相关改动已回退，工作区恢复到锁文件与环境一致的状态。探测超时的修正是这次试验的副产品，与并行本身无关。

0.6.0 的四处声明位置与两处生成文件一致，依赖导出核对通过，CLI 报告 `tool_version` 为 0.6.0，锁文件以 `--no-config` 重新生成因而保留 PyPI 地址。本计划只声明版本号，不构成发布：对应范围的验收、独立审查与支持限制说明尚未完成，未创建 tag。
<!-- /tao:results -->

<!-- tao:section questions -->
## 待确认事项

测试执行时间的优化已在本计划内实测并决定放弃，不再列为未决项；若日后重启，先解决跨 worker 只准备一次运行环境与 coverage 追踪 worker 两项前提，再重新度量。

0.6.0 的发布前提未决：验收范围、独立审查与支持限制说明尚未完成，在此之前不创建 tag。本地 `uv` 配置以镜像为默认索引，重新生成锁文件必须使用 `--no-config`，否则会改写全部地址。
