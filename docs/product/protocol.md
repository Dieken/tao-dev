...
<!-- tao:field acceptance -->
**验收：** 审查开始前明确的新报告输出不改变受检输入；源码、未声明新文件与历史报告变化仍使结果过期，不能排除整个报告目录。旧版本的通过结果不能证明新改动完成；未运行、不可用与不适用均不能显示为通过；复用上次执行结果的检查须显示其实际执行时刻与所依据的输入范围，并按该时刻判定复用时效，整体记录的刷新不使其显示为新鲜；token 估算费用不得冒充实际账单。
