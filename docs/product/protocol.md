...
出版适配采用 [现有 Sphinx 方案](../engineering/decisions/sphinx-publication.md)。客户端版本和加载方式按 [接入指南](../user/clients.md) 选择；质量预算和检查集在各项目接入时确定，见 [项目接入](../../plugins/tao-dev/skills/tao-dev/references/project-setup.md)。
...
<!-- tao:field acceptance -->
**验收：** 一个审查批次自身的记录——其预留的新报告输出、主审裁决、批次导航页，以及计划中记录本次执行的结果块——均不改变该批次的受检输入，使完成一轮审查所必需的产出不会反过来使该轮结论失效；源码、未声明新文件与其他批次的报告变化仍使结果过期，不能排除整个报告目录。旧版本的通过结果不能证明新改动完成；未运行、不可用与不适用均不能显示为通过；复用上次执行结果的检查须显示其实际执行时刻与所依据的输入范围，并按该时刻判定复用时效，整体记录的刷新不使其显示为新鲜；token 估算费用不得冒充实际账单。
...
<!-- tao:field acceptance -->
**验收：** 关联链接显示类型和目标标题，文档页提供自动生成的前向与反向关系；任务的需求及依赖链接可点击，ID 保持稳定。无需手工维护关联表即可形成主题导航和需求索引；发布文档能定位到对应版本；移除 skill 后源文件仍可阅读。
...
在所有讨论中，对用户和作者的重要提议独立分析收益、代价、必要条件与可行替代，给出推荐及改选条件，不限于 spec、design 或正式开发阶段。结合上下文区分候选方案、已确认决定与行动授权，不把试探性提议自动写成要求。审查覆盖文档、代码、测试、配置及 prompt 等相关产物；按后果选择强度，加强审查优先采用不同模型供应商，并保留实际模型、输入、证据、处置和成本记录。

<!-- tao:field acceptance -->
**验收：** 事实不变时，不因用户赞同或反对的表态而无理由翻转判断；重要提议有具体依据和明确推荐，比较可行替代或说明没有可行替代的依据；有实际需求的合理复杂度不被机械拒绝。讨论中的候选不冒充决定或行动授权；可查事实自行核实，问题按决策依赖推进。合理方案可赞同，用户知情决定后无新证据不反复争论，不为日常答疑强制比较或追问。审查者先独立检查，再对照作者理由；跨供应商不可确认或未执行时不得宣称完成；小修改不强制多模型流程，发现问题以证据裁决而非多数票。
...
<!-- tao:field given -->
**给定：** 独立样例包括：局部文案修改；没有必要性证据却被建议增加服务的方案；具有明确隔离或恢复约束的相似方案；含已知缺陷的代码、配置和规格。另含日常操作选择、协作方式建议与排错假设的纯讨论场景，分别使用试探提议、明确决定和已授权行动的表达。为同一事实制作用户赞成与反对两种表达，另测供应商不可用及模型信息未知。
<!-- tao:field when -->
**当：** 运行 skill 进行取舍或审查，并在加强审查样例中使用实际可确认的不同供应商；保留审查者在阅读作者自评和其他审查结论前的独立结果，对照最终处置与可复现证据。
<!-- tao:field then -->
**则：** 建议随事实与约束而非赞同压力变化；重要提议获得具体评估与推荐，可行替代得到比较且不编造选项，纯讨论不触发未获准写入，明确授权不被反复询问。前置决定未定时不暗含其答案追问，合理方案不被强制反对，知情选择后无新依据不重议。不机械拒绝有依据的复杂度，也不为正确产物编造缺陷；局部修改不启动不必要的多模型流程。真实缺陷得到验证和处理，未执行的审查如实记录，意见数量或多数赞同不构成通过。按实际发现、误报与成本评估收益，不能以复述规程判定通过。
```
