...
首版 skill 入口与工程规程已形成独立运行材料，统一格式注册表、可填写模板及通用文档规则已随 skill 提供，见 [插件设计](plugin-design.md)。源文件校验器和独立回归测试已提供；变更骨架生成已实现，出版与自动恢复能力尚待实现；双 CLI 加载及行为验收仍须提供实际证据，流程效率须由使用数据评估。
