...
每份证据关联需求或任务，并记录：受检输入引用（默认固定 VCS 版本、范围及一致性结论；必要时补充差异或快照）、相关契约版本、命令、工具及配置版本、必要环境、时间、退出码、结果摘要与原报告位置。人工验证也记录操作、环境、观察结果与执行者，不虚构退出码。报告保存期限与发布重要性匹配；本地临时文件消失后不得保留虚假的可复核声明。
