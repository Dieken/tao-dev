...
九例监测的个人配置均未改变，临时访问凭据均已删除；认证副本不含刷新令牌，未注册个人插件。项目状态验证、原生 hook 反馈和客户端退出状态是分别核对的证据。该结果仅覆盖上述版本、合成场景与配置，不替代 Windows、Claude 各 scope 或本项目完整独立审查；因此合并了这些条件的任务仍不勾选。
...
后续验收需要原生 Windows 环境，以及可保持个人配置不变的独立审查执行方式。Claude 三种 scope 的加载结果不能替代各 scope 下完整流程的模型行为验收；Codex 的当前保证仅覆盖明确的包格式与已执行案例，不推及其他版本。
