...
Sphinx 扩展在标准目录收集完成后读取已校验的导航图，为每份页面分配由父级位置和同级顺序组成的编号，再从页面目录节点生成内容章节的后续层级。navigation 页本身取得所在篇章编号，但固定的导读和目录标题显式不编号，避免结构性包装制造虚假深度。目录顺序仍由源 toctree 唯一决定；使用方不需要在每份导航文档重复填写出版选项。
